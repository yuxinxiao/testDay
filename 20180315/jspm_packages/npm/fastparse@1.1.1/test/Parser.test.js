...
var testdata = [{
  name: "simple string",
  states: {"start": {"[d-gm-rv]+": function(match, index) {
        if (!this.data)
          this.data = [];
        this.data.push({
          match: match,
...
describe("Parser", function() {
  ...
  it("should default context to empty object", function() {
    var parser = new Parser({"a": {"a": function() {
          this.should.be.eql({});
        }}});
  ...
  it("should error for unexpected format", function() {
    (function() {
      var parser = new Parser({"a": 123});
      return parser;
    }).should.throw();
  });
  it("should error for not existing state", function() {
    var parser = new Parser({"a": {"a": "b"}});
    (function() {
