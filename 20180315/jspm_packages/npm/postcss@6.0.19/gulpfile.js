...
(function(process) {
  ...
  gulp.task('compile', () => {
    ...
    return gulp.src('lib/*.es6').pipe(changed('lib', {extension: '.js'})).pipe(sourcemaps.init()).pipe(babel({
      presets: [['env', {
        targets: {
  ...
  gulp.task('build:package', () => {
    ...
    return gulp.src('./package.json').pipe(editor((json) => {
      delete json.babel;
      delete json.scripts;
      delete json.jest;
  ...
  gulp.task('build:docs', () => {
    let ignore = require('@empty').readFileSync('.npmignore').toString().trim().split(/\n+/).concat(['package.json', '.npmignore', 'lib/*', 'test/*', 'node_modules/**/*', 'docs/api.md', 'docs/plugins.md', 'docs/writing-a-plugin.md']).map((i) => '!' + i);
  ...
  gulp.task('version', ['build:lib'], () => {
    ...
    let pkg = require('./package.json!systemjs-json');
