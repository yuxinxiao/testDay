...
specialFunctions[SCOPE_ATRULE_EXPRESSION] = {url: getUri};
...
function expectAny(what) {
  if (scanner.token !== null) {
    for (var i = 1,
        type = scanner.token.type; i < arguments.length; i++) {
      if (type === arguments[i]) {
        return true;
      }
    }
...
function isBlockAtrule() {
  for (var offset = 1,
      cursor; cursor = scanner.lookup(offset); offset++) {
...
function readAttrselector() {
  expectAny('Attribute selector (=, ~=, ^=, $=, *=, |=)', TokenType.EqualsSign, TokenType.Tilde, TokenType.CircumflexAccent, TokenType.DollarSign, TokenType.Asterisk, TokenType.VerticalLine);
  var name;
