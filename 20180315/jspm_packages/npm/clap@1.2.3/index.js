...
(function(process) {
  ...
  var Option = function(usage, description) {
    var self = this;
    var params;
    var left = usage.trim().replace(/^-([a-zA-Z])(?:\s*,\s*|\s+)/, function(m, name) {
      self.short = name;
      return '';
    ...
    if (arguments.length == 3) {
      if (opt_1 && opt_1.constructor === Object) {
        for (var key in opt_1)
          if (key == 'normalize' || key == 'defValue' || key == 'beforeInit')
  ...
  Command.prototype = {
    ...
    run: function(args, context) {
      ...
      for (var i = 0; i < commands.length; i++) {
        ...
        command.init_(item.args.slice());
        if (item.args.length)
          command.args_(item.args.slice());
        item.options.forEach(function(entry) {
          if (!entry.option.beforeInit)
            command.setOption(entry.option.camelName, entry.value);
        });
    ...
    normalize: function(values) {
      ...
      for (var name in this.values)
        if (hasOwnProperty.call(this.values, name))
          result[name] = hasOwnProperty.call(values, name) && hasOwnProperty.call(this.options, name) ? this.options[name].normalize.call(this, values[name]) : this.values[name];
      for (var name in values)
